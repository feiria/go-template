...
// User 用户表
type User struct {
	ID         uint      `gorm:"autoIncrement:true;primaryKey;column:id;type:int unsigned;not null;comment:'id'" json:"id"`            // id
	...
	Type       uint8     `gorm:"column:type;type:tinyint unsigned;not null;default:1;comment:'账号类型 1-普通用户 2-企业用户 3-内部用户'" json:"type"` // 账号类型 1-普通用户 2-企业用户 3-内部用户
	Status     uint8     `gorm:"column:status;type:tinyint unsigned;not null;default:1;comment:'账号状态 1-开启 2-冻结 3-注销'" json:"status"`   // 账号状态 1-开启 2-冻结 3-注销
